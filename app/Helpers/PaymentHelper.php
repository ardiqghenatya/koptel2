...
class PaymentHelper{
	...
	protected $server_key = "VT-server-6Lku8k5F6qkEz-I0DKtEnse-";

	public function payment($order_id, $order, $save_cc = false){
		
		$VT_Config = new \Veritrans_Config;
		//set server key
		$VT_Config::$serverKey = $this->server_key;

		// Development Environment (the default), use sandbox
		$VT_Config::$isProduction = false;

		// Set sanitization off (default)
		$VT_Config::$isSanitized = false;


		/*==================== dummy data, should have set from $payment variable =========================*/
		$transaction_details = array(
		  'order_id'    => $order_id,
		  'gross_amount'  => $order['order_totals']['grandtotal']
		);

		// Populate items
		$items = array();
		foreach($order['order_products'] as $key => $item){
			$items[$key]['id'] = $item['product_id'];
		...
		//additional cost
		$add_cost = AdditionalCost::find('shipping_handling');
		array_push($items,
			[
				'id' => $add_cost->cost_id,
				'price' => $order['order_totals']['shipping_handling'],
				'quantity' => 1,
				'name' => $add_cost->cost_title
			]
		);
		...
		$shipping_address = array(
		    ...
		    'address'      => $order['shipping_address'][0] ? $order['shipping_address'][0] : $order['shipping_address'][1],
		...
		$token_id = $order['credit_card']['token_id']; //$_POST['token_id'];
	...
	private function callback($response){
		// Success
		$msg = "";
		if($response->transaction_status == 'capture') {
		  $msg = "<p>Transaksi berhasil.</p>";
		  $msg .= "<p>Status transaksi untuk order id $response->order_id: " .
		      "$response->transaction_status</p>";

		  $msg .= "<h3>Detail transaksi:</h3>";
		  $msg .= "<pre>";
		  // var_dump($response);
		  $msg .= "</pre>";
		}
		...
		// Error
		  ...
		  $msg = "<p>Terjadi kesalahan pada data transaksi yang dikirim.</p>";
		  $msg .= "<p>Status message: [$response->status_code] " .
	...
	public function getCountryCode($code, $ISO = 'alpha3'){
		$country_code = "";
		...
		return $country_code ? $country_code : $code;
	}
}